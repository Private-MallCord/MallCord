...
While it was primarily created for MallCord based on Vencord and its respective packages, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @mallcord/discord-types
yarn add -D @mallcord/discord-types
pnpm add -D @mallcord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@mallcord/discord-types";
...
```ts
import { ApplicationCommandType } from "@mallcord/discord-types/enums";
